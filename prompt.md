...
학습자가 지루해할 틈 없이, 표현의 핵심 뉘앙스와 실전 용법을 짧고 강렬한 서사로 전달한다.
...
depth:              {{anchor | support}}
```

## STORY

최대 3~4문단, 각 문단은 2~3문장으로 짧게 치고 빠질 것.
...
조건부다. ETYMOLOGY GATE를 통과할 때만 1문장으로 쓴다.

어원이 뉘앙스를 깊게 하고 기억을 돕는 경우에만, 아주 짧게 덧붙인다. 억지스럽거나 길어지면 통째로 생략하라.
...
유효한 JSON만 반환한다. 코드펜스나 다른 서문은 절대 붙이지 않는다.
...
```json
{
  ...
  "variations": [
    {
      "en": "...",
      "when_ko": "정확히 어떤 특정 상황에 쓰는지 (구체적으로)"
    }
  ],
  "saved_atoms": {
    "headword": "...",
...
- story는 "선택과 공기의 변화" 중심의 1~2문단으로 극단적 압축.
- variations는 반드시 []로 비운다.
- 어원은 생략한다.
