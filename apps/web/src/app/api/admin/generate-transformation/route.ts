...
대상: 영어를 이미 조금은 말하지만, 더 자연스러운 상황별 표현을 익히고 싶은 한국어 화자.
목적: 하나의 패턴을 다양한 상황에서 말해보면서 체화하는 것.
기본 전제: 업무/회의/프로젝트는 수많은 말하기 상황 중 일부일 뿐이다. 트랜스크립트나 speakingFunction이 실제로 그쪽일 때만 그 맥락을 써라.
...
3. 한 가지 배경에 갇히지 않고, 여러 실제 상황으로 옮겨 말하기 쉬운 것
...
- 질문은 인터뷰, 잡담, 소개 자리, 발표 후 질의응답, 1:1 대화 등 실제 상황에서 나올 법한 것
- 업무용일 필요 없음 — 상황이 구체적이면 됨
...
- "새 프로젝트", "업무적으로", "프로페셔널하게" 같은 업무 중심 프레임을 기본값처럼 깔지 말 것.
