...
가장 중요한 질문은 "이 표현이 정확히 어떤 상황에서 제일 잘 먹히는가?"야.
업무/프로젝트/회의는 그 상황들 중 하나일 뿐이다. 트랜스크립트가 실제로 그렇지 않다면 먼저 그쪽으로 몰고 가지 마라.
...
- subtitle은 반드시 '${targetPattern}' 표현을 그대로 포함한 완성된 한국어 문장 (이 표현을 빼거나 바꾸면 안 됨)
`
    : ""
}각 필드 작성 가이드와 예시:

reusable_scenarios — 이 표현/패턴을 실제로 쓸 수 있는 구체적 상황 2-3개. 짧은 한국어 구.
  회사 안팎을 가리지 말고, 이 표현이 가장 자연스럽게 쓰일 장면을 떠올릴 것.
  좋은 예: ["친구한테 근황 설명할 때", "모임에서 의견 다를 때", "발표에서 숫자 언급할 때", "처음 만난 사람과 대화할 때"]
  나쁜 예: ["비즈니스 미팅", "프레젠테이션", "새 프로젝트 논의"] (← 너무 뭉뚱그림)
...
expected_takeaway — 이 세션 후 학습자가 할 수 있는 것 한 문장. 구체적인 상황 + 표현을 넣어라.
  ...
  나쁜 예: "새 프로젝트를 소개할 수 있어요" (← 상황이 너무 한쪽으로 치우침)

subtitle — 완성된 한국어 문장. 변형연습 타깃 패턴이 있으면 반드시 그 패턴을 그대로 포함해야 한다.
  규칙:
