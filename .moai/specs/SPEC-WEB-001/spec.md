...
- **v0.4.0 — 2026-06-30** — ① 질문 캡을 비용 기준→**횟수 기준(하루 콘텐츠당 10회, 월 ~300회)**으로 변경(운영 단순). 토큰비용 추산상 300회/월은 Flash 기준 ~₩200~300으로 비용 안전. ② **가격 정책 최종 확정 = PRD v1.4 연구독**(연 79k/반년 49k/3개월 29k, 월 ₩4,900 폐기). O-2 종료. REQ-WEB-004 갱신.
...
|---|---|---|---|
| C-1 | "월 4,900짜리 웹" | 월 구독 없음. 연 79,000 / 반년 49,000 / 3개월 29,000 **선불 약정**(PRD §4.4) | **v1.4 채택·최종 확정**(사용자 승인 2026-06-30, D16). 월 ₩4,900 폐기 |
...
| D15 | 질문 캡 | **횟수 캡 — 하루 콘텐츠당 10회**(월 ~300회). 소프트: 잔여 긍정형 표시, 소진 시 완곡 안내(다음 날 리셋, 하드 차단 아님). 운영 단순·비용 예측 용이(Flash 기준 300회/월 ≈ ₩200~300 추정) | REQ-WEB-004 |
| D16 | 가격 최종 | **확정: PRD v1.4 연구독**(연 79,000 / 반년 49,000 / 3개월 29,000). 월 ₩4,900 폐기. 사용자 최종 승인(2026-06-30) | REQ-WEB-008 |
...
- **W2 (State-driven, WHILE):** WHILE 사용자의 그날 질문 횟수가 **10회 이하**인 동안, THEN 시스템은 질문을 처리하고 잔여 횟수를 긍정형("오늘 질문 N개 남음")으로 표시해야 한다(D15).
- **U3 (Unwanted, IF/THEN):** IF 학습 화면이 카드·회상 퀴즈·내 표현 사전 큐레이션 surface를 노출하려 한다면, THEN 시스템은 이를 노출하지 않아야 한다(§13 Out, 능동 레이어=질문 히스토리만).
- **U4 (Unwanted, IF/THEN):** IF 사용자의 그날 질문 횟수가 10회 소프트 캡을 초과했다면, THEN 시스템은 질문을 하드 차단하지 않고 경량 모델로 강등하거나 완곡 안내(다음 날 리셋)를 표시해야 한다(D15).
