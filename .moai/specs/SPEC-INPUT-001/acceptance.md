...
### 엣지 케이스
- **EC-004-A (월 100회 소프트 캡 소진 → 강등+안내):** 이번 달 질문이 100회를 소진하면 추가 질문은 graceful하게 처리되며(흐름 차단·크래시 없이) 경량 모델로 강등되거나 다음 달 리셋 안내가 표시된다. 하드 차단(에러·블록)은 없다 (REQ-INPUT-004-U3, D3).
- **EC-004-B (Gemini Flash-gate):** 짧은 답 응답은 `gemini-2.5-flash`(Flash-gate), 복잡 질문은 `gemini-2.5-pro`로 승급한다. Azure는 폴백 (REQ-INPUT-004-U2, D1).
