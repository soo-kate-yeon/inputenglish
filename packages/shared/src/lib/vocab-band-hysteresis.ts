...
export function applyHysteresisVote(
  ...
  const newDirection = vote;

  // Case 3: Check if K is reached → attempt flip
  if (newCount >= k) {
    const bandIdx = BAND_ORDER.indexOf(currentBand);
    const targetIdx = bandIdx + vote; // +1 = raise, -1 = lower

    // Case 4: Clamp — flip that would exceed valid range is a no-op but resets
    if (targetIdx < 0 || targetIdx >= BAND_ORDER.length) {
      return {
        band: currentBand,
        voteState: { direction: 0, count: 0 }, // reset on no-op clamp
        flipped: false,
      };
    }

    const newBand = BAND_ORDER[targetIdx];
  ...
  return {
    band: currentBand,
    voteState: { direction: newDirection, count: newCount },
    flipped: false,
  };
}
