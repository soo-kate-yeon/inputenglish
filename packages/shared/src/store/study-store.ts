...
interface StudyStoreState {
  ...
  updateSessionPhase: (
    sessionId: string,
    phase: StudySession["currentPhase"],
  ) => void;
...
export const createStudyStore = (storage?: StateStorage) =>
  create<StudyStoreState>()(
    persist(
      (set) => ({
        sessions: [],
        currentSession: null,
                ...
                ? {
                    ...
                    sentences: session.sentences.map((sentence) =>
                      sentence.id === sentenceId
                        ? {
                            ...sentence,
                            highlights: sentence.highlights.filter(
                              (h) => h.id !== highlightId,
                            ),
                          }
                        : sentence,
